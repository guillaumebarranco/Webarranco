...
$(document).ready(() => {
	...
	const typewriter = (letexte, check, cmpt, koko) => {

		lelien = document.getElementById('warp');

		if(check === 0) { let courant; }
	...
	function checkCode(e, counter, tab_enter, callback) {
		...
		for (let i = 0; i < tab_enter.length; i++) {
	...
	*/
	...
	const makeW = () => {
		...
		for (let i = 0; i < $('.bloc-about-manga').length; i++) {
		...
		for (let j = 0; j < $('.bloc-about-film').length; j++) {
